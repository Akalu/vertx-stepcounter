...
public class ApiTest {
  ...
  void prepareDb(Vertx vertx, VertxTestContext testContext) {
    String insertQuery = "INSERT INTO step VALUES($1, $2, $3::timestamp, $4)";
    ...
    pgPool.query("DELETE FROM step")
      .rxExecute()
      .flatMap(rows -> pgPool.preparedQuery(insertQuery).rxExecuteBatch(data))
      .ignoreElement()
      .andThen(vertx.rxDeployVerticle(new StepApiVerticle()))
      .ignoreElement()
  ...
  void checkRanking24Hours() {
    ...
    assertThat(data.get(0))
      ...
      .containsEntry("count", 2500);
    assertThat(data.get(1))
      .containsEntry("deviceId", "def")
      .containsEntry("count", 1000);
  }
}
