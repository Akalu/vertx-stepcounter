...
import verticle.data.SqlQueries;

import java.time.DateTimeException;
import java.time.LocalDateTime;

@Slf4j
public class StepApiVerticle extends AbstractVerticle {
	...
	private void totalSteps(RoutingContext ctx) {
		...
		pgPool.preparedQuery(SqlQueries.totalStepsCount()).rxExecute(params).map(rs -> rs.iterator().next())
				.subscribe(row -> sendCount(ctx, row), err -> handleError(ctx, err));
	}

	private void sendCount(RoutingContext ctx, Row row) {
		Integer count = row.getInteger(0);
		if (count != null) {
			JsonObject payload = new JsonObject().put("count", count);
	...
	private void stepsOnMonth(RoutingContext ctx) {
		try {
			String deviceId = ctx.pathParam("deviceId");
			LocalDateTime dateTime = LocalDateTime.of(Integer.parseInt(ctx.pathParam("year")),
					Integer.parseInt(ctx.pathParam("month")), 1, 0, 0);
			Tuple params = Tuple.of(deviceId, dateTime);
			pgPool.preparedQuery(SqlQueries.monthlyStepsCount()).rxExecute(params).map(rs -> rs.iterator().next())
					.subscribe(row -> sendCount(ctx, row), err -> handleError(ctx, err));
	...
	private void stepsOnDay(RoutingContext ctx) {
		try {
			String deviceId = ctx.pathParam("deviceId");
			LocalDateTime dateTime = LocalDateTime.of(Integer.parseInt(ctx.pathParam("year")),
					Integer.parseInt(ctx.pathParam("month")), Integer.parseInt(ctx.pathParam("day")), 0, 0);
			Tuple params = Tuple.of(deviceId, dateTime);
			pgPool.preparedQuery(SqlQueries.dailyStepsCount()).rxExecute(params).map(rs -> rs.iterator().next())
					.subscribe(row -> sendCount(ctx, row), err -> handleError(ctx, err));
		} catch (DateTimeException | NumberFormatException e) {
			sendBadRequest(ctx);
		}
	...
	private void ranking(RoutingContext ctx) {
		pgPool.preparedQuery(SqlQueries.rankingLast24Hours()).rxExecute().subscribe(rows -> sendRanking(ctx, rows),
	...
	private void sendRanking(RoutingContext ctx, RowSet<Row> rows) {
		...
		for (Row row : rows) {
			data.add(new JsonObject().put("deviceId", row.getValue("device_id")).put("count",
					row.getValue("steps")));
		}
